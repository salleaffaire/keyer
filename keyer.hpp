...
void apply_key_sse2(uint8_t *bg, uint8_t *key, uint8_t *alpha, int width,
  ...
  for (uint32_t i = 0; i < num_pixels; i += pixel_group) {
    ...
    __m128i curBgGroup =
        *(__m128i *)(&bg[i * pixel_group]);  // Load 4 Background pixels
    __m128i curKeyGroup =
        *(__m128i *)(&key[i * pixel_group]);  // Load 4 Key pixels
    __m128i curAlphaGroup =
        *(__m128i *)(&alpha[i * pixel_group]);  // Load 4 Alpha pixels
...
void apply_key_avx(uint8_t *bg, uint8_t *key, uint8_t *alpha, int width,
  ...
  for (uint32_t i = 0; i < num_pixels; i += pixel_group) {
    ...
    __m256i curBgGroup =
        *(__m256i *)(&bg[i * pixel_group]);  // Load 4 Background pixels
    __m256i curKeyGroup =
        *(__m256i *)(&key[i * pixel_group]);  // Load 4 Key pixels
    __m256i curAlphaGroup =
        *(__m256i *)(&alpha[i * pixel_group]);  // Load 4 Alpha pixels
